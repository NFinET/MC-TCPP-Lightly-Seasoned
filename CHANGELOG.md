...
LambDynamicLights | Allows you to see / use dynamic lights when holding items that create light, and works with or without shaders! Important to note, this light is only rendered and not "real", and thus doesn't count for lighting-based calculation mechanics, like spawning of mobs. **This feature is turned off by default for the benefit of people with weak PCs or who don't want to use it. It can be enabled by finding LambDynamicLights' settings in the Mods menu, or by using the Puzzle Button in the Options menu and going to the Graphics Settings.**
